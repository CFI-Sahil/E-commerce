...
import { categoriesData } from "../Products_details/SimData";
...
const Similar = () => {
  ...
  // We use this ref to track if the mouse actually moved during the click
  ...
  // ... (Keep your existing useEffect for Resize logic here) ...
  useEffect(() => {
  ...
  // ... (Keep your existing handleScroll and scroll logic here) ...
  const handleScroll = () => {
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    isDragMove.current = false; // Reset drag check
    setIsDragging(true);
  ...
  const handleMouseUp = () => {
    ...
    // Note: We don't reset isDragMove here, we check it in onClick
  };
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    // If we move the mouse, mark it as a drag
  ...
  return (
    <div className="font-primary text-text select-none">
      ...
      <div className="flex justify-between items-center px-2 md:px-8 lg:px-20 mt-6">
        <p className=" text-md lg:text-xl font-semibold">Similar Products</p>
        <a href="#" className="flex items-center justify-center gap-x-1 text-[12px] lg:text-sm text-primary">
      ...
      <div className="relative px-2 md:px-8 lg:px-20 mt-2.5">
        {!isTablet && showLeft && (
          <button onClick={() => scroll("left")} className="absolute left-14 top-1/2 -translate-y-1/2 bg-white p-2 rounded shadow z-10">
        ...
        >
          {categoriesData.map((item) => (
            <div
              key={item.id}
              className="shrink-0 transition-transform active:scale-95" // Added a small click effect
            ...
            >
              <div className="w-full h-[280px] md:h-[250px] lg:h-[260px] xl:h-[300px] 2xl:h-[350px] rounded-md shadow flex flex-col justify-center pt-2 pb-3.5 px-1.5 border border-border bg-white">
                ...
                <div className="flex gap-2 items-center">
                  <p className="text-offer text-[12px] font-medium">{item.offer}</p>
                  <p className="line-through text-[12px] text-priceline">{item.line}</p>
                </div>
              </div>
            </div>
          ))}
        ...
        {!isTablet && showRight && (
          <button onClick={() => scroll("right")} className="absolute right-14 top-1/2 -translate-y-1/2 bg-white p-2 rounded shadow z-10">
